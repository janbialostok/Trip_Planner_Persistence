...
$(document).ready(function () {
	...
	generateAttraction = function (config) {
		config.$all.find('.add').on('click', function () {
			...
			var attrObj = { attrId: newAttraction._id };
		...
		return {
			...
			buildItineraryItem: function () {
				...
				jQuery.data($button, "id", this._id);
				...
				$button.on('click', function () {
					self.delete();
					var dayNum = $('#day-title span').text().split(' ')[1];
					var url = '/day/' + dayIds[dayNum-1] + '/' + self.attractionType;
					$.ajax({ type: "DELETE", url: url, data: { aid: self._id }, success: function(data){
						console.log(data);
					}});
				});
				return this;
